...
[?php slot('ua5_admin.current_header') ?]
<th class="ua5_admin_<?php echo strtolower($field->getType()) ?> ua5_admin_list_th_<?php echo $name ?>">
<?php if ($field->isReal()): ?>
  [?php if ('<?php echo $name ?>' == $sort[0]): ?]
    ...
    [?php echo image_tag(sfConfig::get('ua5_admin_module_web_dir').'/images/'.$sort[1].'.png', array('alt' => __($sort[1], array(), 'ua5_admin'), 'title' => __($sort[1], array(), 'ua5_admin'))) ?]
  [?php else: ?]
...
[?php end_slot(); ?]
<?php echo $this->addCredentialCondition("[?php include_slot('ua5_admin.current_header') ?]", $field->getConfig()) ?>
<?php endforeach; ?>
